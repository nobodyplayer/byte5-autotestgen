...
def initialize_llm(provider, llm_model_name, embedding_model_name) -> Tuple[Optional[BaseChatModel], Optional[Embeddings]]:
    """
    初始化LLM模型和嵌入模型。
    ...
    embeddings_class = import_class(LLM_PROVIDER_CONFIG[provider]["embeddings_module"], LLM_PROVIDER_CONFIG[provider]["embeddings_class"])
    if not LLM_class:
        print("未找到对应的LLM模型类")
        return None, None
    try:
